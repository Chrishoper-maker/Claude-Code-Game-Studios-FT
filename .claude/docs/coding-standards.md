...
- Engine-specific CI commands:
  - **Godot**: `godot --headless --import` 然后 `godot --headless -s res://addons/gdUnit4/bin/GdUnitCmdTool.gd --ignoreHeadlessMode --continue -a res://tests`（GdUnit4；必先 --import 建全局类名缓存；--ignoreHeadlessMode 跳过 GdUnit4 的无头拒绝）
