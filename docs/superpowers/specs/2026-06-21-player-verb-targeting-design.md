...
- **cannon**：定向穿透，选"方向"而非"目标格"，UX 本质不同 → **本增量不含，单列 story**（保留 cannon 的 push_warning）。
...
**AC-5：cannon 仍未支持（保留警告）**【单元】选中炮手 → do_verb → 未进 VERB（mode 仍 IDLE）、`has_used_verb` 仍 false。

**AC-6：全量回归绿**。

## 范围/偏离

- 仅 aura/heal/displace；**cannon 定向选靶单列后续 story**。
- heal 不含自疗（相邻排除自身）。
