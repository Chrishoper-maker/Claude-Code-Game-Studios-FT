...
### 3.5 招募获取（②a 暂行，待 ②c 取代）
- 招募候选随机装备：滚**一件普通装备**（限 `rarity==0` 子池，仿现有 `_offer_equipment`），按其 `slot` 记入该船员对应槽，其余槽空。
- 起始船员仍无装备（全槽空）。
- 此渠道是地基期占位；真正的多件收集（按 tier 掉不同稀有度、按槽分配）由 ②c 战利品经济提供。
...
- **某槽无装**：访问器跳过该槽，不计增量。
- **高稀有度不覆盖全槽**：合法——某些槽无传奇/稀世装备；查询返回空子池时调用方（②c 后续）自行降级，②a 招募只取普通故不触发。
...
- **AC-4**：招募滚一件普通装备并按 slot 装入；`get_recruit_offers` 滚出的装备 `rarity` 恒为 0。
...
- **AC-7**（集成）：招募带普通装备 → 部署 → 战斗有效值反映该槽增量，端到端经 RunManager 驱动。
...
- **逻辑（BLOCKING，单测）**：
  ...
  - `get_recruit_offers` 滚装限普通（rarity==0）+ 按 slot 记账。
  - `get_equipment_for` 返回槽字典。
  - `to_save_dict`/`load_from_save_dict`：槽字典往返 + **旧档单 id 迁移** + 双过滤。
- **集成（BLOCKING）**：招募带普通装备 → 部署 → 战斗有效值反映增量（AC-7）。
