...
- **滚动同槽约束**：8 件中**同一槽最多出现 2 件**（保证槽位变化、供同槽二选一）。滚出的件若会使某槽达 3 件，则重滚该件（换槽/换件），直至满足约束或可滚件耗尽。
...
- **滚 8 件同槽 ≤2**：滚动期约束每槽至多 2 件；触顶则重滚该件。
- **8 件含重复（有放回滚出同一 id）**：允许（同 id 必同槽，受同槽 ≤2 与选 2 不同槽规则共同约束）。
...
- **AC-4**：招募滚 8 件（按权重稀有度抽、子池空降级、**同槽 ≤2**），玩家选 2 件（限不同槽）各按 slot 装入；选装确定性（同 `rng_state` 滚出同 8 件）。
...
- **逻辑（BLOCKING，单测）**：
  ...
  - 招募滚 8 件：权重稀有度分布（大样本统计近似 2/8/15/25/50）、子池空降级、**同槽 ≤2 约束**、确定性（同 seed 同 8 件）。
